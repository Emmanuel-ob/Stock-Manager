...
class UserAccountForm(forms.ModelForm):
    gender              =   forms.CharField(max_length= 1, help_text="Select your gender", widget=forms.TextInput(attrs= {'class' : 'form-control', 'placeholder' : 'Gender', 'required' : 'required'}))
    phone_number        = forms.CharField(max_length = 120, help_text = '', widget = forms.TextInput(attrs = {'class': 'form-control', 'placeholder': 'PhoneNumber', 'required': 'required'}))
    date_of_birth       =   forms.DateField(help_text="", widget=forms.TextInput(attrs= {'class' : 'form-control', 'placeholder' : '22/04/1990', 'required' : 'required'}))
    ...
    class Meta:
        model = UserAccount
        fields = ( 'gender',  'phone_number', 'date_of_birth', 'address', 'state', 'country')
